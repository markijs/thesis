...
\begin{figure}[t]
	...
	\begin{subfigure}[b]{.49\linewidth}
		\begin{tikzpicture}
		\centering
		\node[inner sep=0] (image) {\includegraphics[width=6cm] {cavity7.pdf}};;
		\end{tikzpicture}
		\centering
		\caption{}
		\label{a}
	...
	\begin{subfigure}[b]{.49\linewidth}
		\begin{tikzpicture}
		\centering
		\node[inner sep=0] (image) {\includegraphics[width=6cm] {glue.pdf}};;
		\end{tikzpicture}
		\centering
		\caption{}
		\label{b}
	...
	\begin{subfigure}[b]{.49\linewidth}
		\begin{tikzpicture}
		\centering
		\node[inner sep=0] (image) {\includegraphics[width=6cm] {cavity5.pdf}};;
		\end{tikzpicture}
		\centering
		\caption{}
		\label{b}
	...
	\begin{subfigure}[b]{.49\linewidth}
		\begin{tikzpicture}
		\centering
		\node[inner sep=0] (image) {\includegraphics[width=6cm] {cavity.pdf}};;
		\end{tikzpicture}
		\centering
		\caption{}
		\label{b}
...
\begin{figure}[t!]
	...
	\node[box, markrood] (fsr) at (0,0) {\footnotesize\sc free spectral r.\\[0.20em] \footnotesize $ \Delta \nu_{\mathrm{FSR}} $};
	\node[box, markrood] (nuspace) at (9,0) {\footnotesize\sc mode spacing \\[0.20em] \footnotesize $ \nu_{q,1,0} - \nu_{q,0,0} $};
	\node[box] (d2L) at (0,-2) {\footnotesize\sc cavity length \\[0.20em] \footnotesize $ d = 2L $};
	\node[box] (deltaPsi) at (6.75,-2) {\footnotesize\sc Gouy phase shift \\[0.20em] \footnotesize $ \Delta \Psi_x $};
	\node[box] (zR) at (4.5,-4) {\footnotesize\sc Rayleigh length \\[0.20em] \footnotesize $ z_{\mathrm{R},x} $};
	\node[box, markrood] (lambda) at (0,-4) {\footnotesize\sc wavelength \\[0.20em] \footnotesize $ \lambda $};
	\node[box] (w0) at (2.25,-6) {\footnotesize\sc mode waist \\[0.20em] \footnotesize $ w_{0,x} $};
	\node[box] (Rc) at (0,-8) {\footnotesize\sc mirror roc \\[0.20em] \footnotesize $ R_{\mathrm{c},x} $};
	
	% Horizontal line between FSR and transverse mode spacing
	\draw[line] (fsr.east) -- (nuspace.west);
	...
	\draw[arrow] (fsr.south) -- (d2L.north) node[label, right, xshift=2mm, yshift=3mm] {\sc def.};
	...
	\caption[Flowchart for determining mode and mirror geometry from spectral data]{%
		Flowchart illustrating how the mode and mirror geometry parameters of an optical cavity can be inferred from spectral measurements (red). Referenced equations used at each step can be found in the main text. In the case of elliptical mirrors, the radii of curvature differ along the $x$ and $y$-axes, resulting in non-degenerate transverse modes: $\nu_{q,1,0} \neq \nu_{q,0,1}$. In this flowchart, the mode and mirror parameters are considered along the $x$-axis, as indicated by the subscripts: $\Delta\Psi_x$,  $z_{\mathrm{R},x}$, $w_{0,x}$ and $R_{\mathrm{c},x}$.%
	}
...
\end{equation}
With a mode waist $ w_0 = \SI{13.4}{\micro\meter} $ and resonator length $ L = \SI{476}{\micro\meter}$, \autoref{eq:gmax} yields a maximum atom-cavity coupling strength of
\begin{equation}
