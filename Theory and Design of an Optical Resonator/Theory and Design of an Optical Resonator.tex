...
\end{scope}
\end{tikzpicture}
\caption[Optical setup for generation of dipole traps and MOT]{Ince--Gaussian modes as a continuous mode family that interpolates between Laguerre--Gaussian ($\mathcal{L}_{n,l}$) and Hermite--Gaussian ($\mathcal{H}_{n_x,n_y}$) modes. This transition is parametrised via the ellipticity parameter $\epsilon$. \textsc{(right)} The even Ince--Gaussian modes $\mathcal{I}_{4,m}^\mathrm{e}(\vec{r}, \epsilon)$ are shown for $\epsilon\in\{0,2,\infty\}$ and $m\in\{0,2,4\}$. Using the HSV representation \textsc{(top left)}, the value and hue correspond to the amplitude and phase of these modes, respectively. The mode amplitudes are scaled such that the fundamental Gaussian mode attains a maximum of unity (not shown). As $\epsilon\rightarrow 0$ or $\epsilon\rightarrow \infty$, the Ince--Gaussian modes reveal a direct correspondence to the Laguerre--Gaussian and Hermite--Gaussian mode families, respectively \textsc{(bottom left)}.}
\label{incegauss} 
\end{figure}
...
\begin{wrapfigure}[12]{l}{0.5\textwidth}
	...
	\end{axis}
	\end{tikzpicture}
	\caption[Measured transmission spectrum of the Fabry--Pérot resonator]{Measured transmission spectrum of the Fabry--Pérot resonator, showing a central resonance peak fitted with a Lorentzian profile. Modulation sidebands appear at $\pm \SI{25}{\mega\hertz}$ and serve as a precise frequency reference to extract the FWHM of the central resonance peak: $\Delta \nu_\mathrm{A} = \SI{4.20\pm0.03}{\mega\hertz}$.}
	\label{cavitytrace}
\end{wrapfigure}
