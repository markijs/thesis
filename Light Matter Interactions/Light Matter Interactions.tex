...
\end{equation}
Since this force depends on $\Delta$, it has a velocity dependent component due to the Doppler shift of moving atoms, which is the underlying principle of Doppler cooling (see \autoref{dopplercooling}). In addition, the scattering force may acquire a positional dependence in the presence of a spatially varying magnetic field, which alters $\Delta$ via the Zeeman effect. This position-dependent force, in combination with Doppler cooling, is what allows a magneto-optical trap (MOT) to cool and confine atoms (see \autoref{mots}).
