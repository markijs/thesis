...
where $\hat{\rho}$ is the time-dependent density operator\footnote{Also in the rotating frame, defined as $\hat{\rho}(t)=\hat{R}(t)\hat{\rho}'(t)\hat{R}^\dagger(t)$, where $\hat{\rho}'(t)$ is the density operator in the non-rotating frame and $\hat{R}(t)=\exp(i\omega\hat{\sigma}_z t/2)$.} of the atom,
 $\hat{\sigma}_z=\vert e \rangle \langle e \vert-\vert g \rangle \langle g \vert$, $\hat{\sigma}_x=\vert e \rangle \langle g \vert+\vert g \rangle \langle e \vert$, $\hat{\sigma}_+=\vert e \rangle \langle g \vert$, $\hat{\sigma}_-=\vert g \rangle \langle e \vert$ and $\gamma$ is the transverse decay rate, which in the case of purely radiative damping equals half the natural decay rate $\Gamma$ of the excited state: $\gamma=\sfrac{\Gamma}{2}$. Note that \autoref{lindblad} features a competition between the coherent dynamics induced by the light field (first two terms) and the dissipative dynamics that arise from the decay into the vacuum of the radiation field (last three terms). Typically, \autoref{lindblad} is rewritten as a system of coupled differential equations (optical Bloch equations), from which one can extract the steady state ($\frac{\partial\hat{\rho}}{\partial t}=0$) solution for ${\rho}_{ee}=\langle e \vert \hat{\rho} \vert e\rangle$, which represents the equilibrium population in the excited state:
\begin{equation}
\label{rhoee}
{\rho}_{ee}=\frac{\Omega^2}{\Gamma^2+4\Delta^2+2\Omega^2}\,.
\end{equation}
The scattering rate $R_\text{sc}$ of spontaneously emitted photons is the product of the population in the excited state and the natural decay rate: $R_\text{sc}=\Gamma \rho_{ee}$. Since the laser intensity is given by $I=\frac{1}{2}\epsilon_0 c E_0^2$, we can rewrite the scattering rate as
\begin{equation}
\label{scatter}
...
\begin{align}
I_\text{sat}:=\frac{c\epsilon_0\Gamma^2\hbar^2}{4\lvert \vec{d}_{eg}\cdot\vec{\epsilon}\, \rvert^2}\,,
\end{align}
such that $2\Omega^2/\Gamma^2=I/I_\text{sat}$. Note from \autoref{rhoee} that for $\Omega^2\gg\Gamma^2,\Delta^2$, we have $R_\text{sc}\approx\sfrac{\Gamma}{2}$, which is the saturation limit. Dipole trapping lasers typically operate at a large detuning ($\Delta^2\gg\Omega^2,\Gamma^2$), in which case $R_\text{sc}\approx\sfrac{\Gamma\Omega^2}{4\Delta^2}$. The scattering force is given by the product of the photon momentum $\hbar k$ and the scattering rate:
\begin{equation}
F_\text{sc}(\Delta)=\hbar kR_\text{sc}(\Delta)\,.
\end{equation}
...
Doppler cooling is a technique used to reduce the kinetic energy of atoms, which enables their confinement in conservative potentials like optical dipole traps (see \autoref{dipoleforce}). When an atom travels at a velocity $v$ facing a counter-propagating laser with wavenumber $k$, the angular frequency $\omega$ of the incident laser light is Doppler-shifted by $kv$: $\omega \rightarrow \omega + kv$. In a one-dimensional setup with two counter-propagating laser beams, the net scattering force $F_\text{net}$ acting on a moving atom can thus be expressed\footnote{This description is only valid for laser intensities well below saturation ($I/I_\text{sat}\ll 1$), in which case the scattering from each beam can be treated independently.} and expanded around $v=0$ as:
\begin{align}
\begin{split}
	F_\text{net} &= F_\text{sc}(\Delta+kv) - F_\text{sc}(\Delta-kv) \\&= -\underbrace{\frac{16  \hbar k^2 \Gamma\Delta\Omega^2 }{(\Gamma^2 + 4 \Delta^2 +  2 \Omega^2)^2}}_{\alpha}v + \mathcal{O}(v^3) \\&\approx -\alpha v\,.
\end{split}
\label{coolingforce}
\end{align}
...
%For $\Delta=\sfrac{\Gamma}{2}$ and $\Omega=\Gamma$, the damping coefficient $\alpha$ attains its maximum of $\alpha_\text{max}=\sfrac{\hbar k^2}{2}$.
\begin{figure}[!t]
	\hspace{-0.4em}
	\begin{tikzpicture} 
	...
	ylabel={$\sfrac{F_\text{net}}{\hbar k \Gamma}$},
	%axis lines = middle,
	grid = major,
	...
	\end{groupplot}
	\end{tikzpicture}
	\caption[Damping coefficient and net cooling force in a one-dimensional optical molasses]{\textsc{(left)} Dimensionless damping coefficient $\sfrac{\alpha}{\hbar k^2}$ in a one-dimensional optical molasses as a function of $\sfrac{\Omega}{\,\Gamma}$ and $\sfrac{\Delta}{\,\Gamma}$. For fixed values of $\sfrac{\Omega}{\,\Gamma}$, the minima and maxima are shown as white and black dashed lines, respectively. \textsc{(right)} Net cooling force $\sfrac{F_\text{net}}{\hbar k \Gamma}$ versus velocity $\sfrac{v}{(\Gamma k^{-1} )}$ (both dimensionless) for ${\Omega}=\sfrac{\Gamma}{2}$ and $\Delta\in\{\sfrac{\Gamma}{10},\sfrac{\Gamma}{2}, \Gamma, 2\Gamma\}$. The antisymmetric profile is characteristic of Doppler cooling, exhibiting a linear dependence near $v = 0$ and saturation at higher velocities. Dashed curves represent the forces exerted by a single cooling beam.
	}
	\label{dopplerplots} 
\end{figure}
The kinetic energy cooling rate that corresponds to this damping force can be written as
\begin{align}
	\frac{\mathrm{d} \langle E_\text{c} \rangle}{\mathrm{d}t}=\langle F_\text{net}v\rangle\approx-\alpha\langle v^2 \rangle  \,,
\label{coolingrate}
\end{align}
where the angle brackets denote the ensemble average. While the cooling force systematically reduces the atomic velocity, each spontaneously emitted photon (associated with a scattering event) transfers a momentum of magnitude $\hbar k$ to the atom in a random direction, which increases the kinetic energy by the recoil energy $E_\text{r}=\sfrac{\hbar^2 k^2}{2m}$. This stochastic process of recoil heating ultimately limits the minimum temperature $T_\text{D}$ achievable through Doppler cooling. The kinetic energy heating rate can be written as \cite{cohen1992}
\begin{align}
\frac{\mathrm{d} \langle E_\text{h} \rangle}{\mathrm{d}t}=4 E_\text{r} R_\text{sc} \,.
\label{heatingrate}
\end{align}
To find an expression for $T_\text{D}$, we use the fact that at equilibrium, the sum of the Doppler cooling rate (\autoref{coolingrate}) and the recoil heating rate (\autoref{heatingrate}) equals zero. This allows us to write the following expression for the average kinetic energy:
\begin{align}
	 \frac{1}{2}m\langle v^2\rangle \approx \frac{\hbar^2 k^2 R_\text{sc}}{\alpha}= \frac{\hbar\left(\Gamma^2+4\Delta^2+2\Omega^2\right)}{16\Delta}\,,
\label{dopplerlimit}
\end{align}
which attains a minimum of $\frac{\hbar\Gamma}{4}\sqrt{1+I/I_\text{sat}}$ for $\Delta=\frac{\Gamma}{2}\sqrt{1+I/I_\text{sat}}$. Using the fact that $\frac{1}{2}m\langle v^2\rangle = \frac{1}{2}k_\text{B}T$ and $I/I_\text{sat}\ll 1$, we arrive at the following result:
\begin{align}
	k_\text{B}T_\text{D} \approx \frac{\hbar\Gamma}{2}\,,
\end{align}
which is known as the Doppler cooling limit. For \ce{^{87}Rb}, $T_\text{D}=\SI{146}{\micro\kelvin}$.
...
\end{align}
where $g_F$ is the Landé $g$-factor and $\mu_B$ is the Bohr magneton. Since counter-propagating beams have opposite (circular) polarisations, the atoms displaced from the trap centre preferentially absorb photons from the beam with the smallest detuning from the relevant transition ($m_F'-m_F= \pm1$ for $\sigma_\pm$). The imbalance in scattering forces for opposing beams results in a net restoring force that directs the atoms back towards the trap centre.
...
\begin{align}
\begin{split}
	\hat{\mathcal{H}}_I &= -\hat{\vec{d}} \cdot \vec{E} \\ &=-\sum_{j} \left( \langle g \vert \hat{\vec{d}}\, \vert e_j \rangle \vert g \rangle \langle e_j \vert +  \langle e_j \vert \hat{\vec{d}}\, \vert g \rangle \vert e_j \rangle \langle g \vert \right)  \\ &\hspace{12em} \times \left( \hat{\epsilon}E_0^{(+)} \mathrm{e}^{-i\omega t} + \hat{\epsilon}^*E_0^{(-)} \mathrm{e}^{i\omega t} \right) \,,
\end{split}
\end{align}
...
\end{split}
\end{align}
where the eigenvalues with respect to \autoref{hyperfinehamiltonian} have been obtained by writing the $\hat{J}_x$ and $\hat{J}_y$ components of $\hat{\vec{J}}$ in terms of the raising and lowering operators as $\hat{J}_x=(\hat{J}_+ + \hat{J}_-)/2$ and $\hat{J}_y=(\hat{J}_+ - \hat{J}_-)/2i$, before using the fact that $\langle j\, m \vert \hat{J}_\mp \hat{J}_\pm \vert j\, m \rangle = \hbar^2\left( j(j+1) - m(m\pm1)\right)$. For intermediate field strengths, when neither $\hat{\mathcal{H}}_\text{s}$ nor $\hat{\mathcal{H}}_\text{hfs}$ dominates, both perturbative approaches break down. In this case, the full Hamiltonian $\hat{\mathcal{H}}=\hat{\mathcal{H}}_\text{s}+\hat{\mathcal{H}}_\text{hfs}$ must be diagonalised numerically. Some progress can be made however, by considering the Hamiltonian in the hyperfine basis $\vert J\, I\, F\, m_F\rangle$. Let us assume, for the sake of simplicity, that the field is linearly polarised along the $\hat{z}$-axis: $(E_0^{(+)})_0=E_0^{(+)}$. The matrix elements of $\hat{\mathcal{H}}_\text{s}$ are thus
\begin{align}
...
\begin{align}
\label{uncoupledham}
\hat{\mathcal{H}}_0=\hbar \omega_0 \vert e \rangle \langle e \vert + \hbar \omega \left( \hat{a}^\dagger_{\vec{k},\hat{\epsilon}} \hat{a}_{\vec{k},\hat{\epsilon}} + \sfrac{1}{2} \right)\,,
\end{align}
...
\fi
\iffalse
In the electric dipole approximation, the electromagnetic field is assumed to be uniform over the size of the atom. This reduces the electric dipole interaction to $\hat{\mathcal{H}}_I=-\hat{\vec{d}}\cdot \hat{\vec{E}}(\vec{0}\,)$.
\fi
In the interaction picture, $\hat{\mathcal{H}}_I$ reads:
\begin{align}
\label{interactionhamiltonianallterms}
\begin{split}
\hat{\mathcal{H}}_I(t) &= \mathrm{e}^{i\hat{\mathcal{H}}_0 t/\hbar}\hat{\mathcal{H}}_I \mathrm{e}^{-i \hat{\mathcal{H}}_0 t/\hbar} \\&= \hbar \Big( g_{\vec{k},\hat{\epsilon}} \hat{\sigma}_+ \hat{a}_{\vec{k},\hat{\epsilon}} \mathrm{e}^{i(\omega_0-\omega)t} + g^*_{\vec{k},\hat{\epsilon}} \hat{\sigma}_- \hat{a}^\dagger_{\vec{k},\hat{\epsilon}} \mathrm{e}^{-i(\omega_0-\omega)t} \\ &\hspace{4em}+ g_{\vec{k},\hat{\epsilon}} \hat{\sigma}_+ \hat{a}^\dagger_{\vec{k},\hat{\epsilon}} \mathrm{e}^{i(\omega_0+\omega)t} + g^*_{\vec{k},\hat{\epsilon}} \hat{\sigma}_- \hat{a}_{\vec{k},\hat{\epsilon}} \mathrm{e}^{-i(\omega_0+\omega)t}     \Big)\,,
\end{split}
\end{align}
where the atom-cavity coupling rate $g_{\vec{k},\hat{\epsilon}}(\vec{r}\,)$ is defined as
\begin{align}
\label{couplingrate}
g_{\vec{k},\hat{\epsilon}}(\vec{r}\,) = -\sqrt{\frac{\omega}{2\epsilon_0\hbar}} \langle e \vert \hat{\vec{d}} \, \vert g \rangle \cdot \vec{f}_{\vec{k},\hat{\epsilon}}(\vec{r}\,)\,.
\end{align}
...
\begin{align}
\label{JC}
\hat{\mathcal{H}}=\hbar\omega_0 \vert e \rangle \langle e \vert + \hbar \omega \hat{a}^\dagger_{\vec{k},\hat{\epsilon}} \hat{a}_{\vec{k},\hat{\epsilon}} + \hbar g_{\vec{k},\hat{\epsilon}}(\hat{\sigma}_+\hat{a}_{\vec{k},\hat{\epsilon}} + \hat{\sigma}_-\hat{a}^\dagger_{\vec{k},\hat{\epsilon}})\,,
\end{align}
...
\begin{align}
\hat{\mathcal{H}}_n \begin{bmatrix}  \vert e, n-1\rangle \\ \vert g, n\rangle \end{bmatrix} = \hbar \begin{bmatrix} (n-1)\omega + \omega_0 & g\sqrt{n} \\ g\sqrt{n} & n\omega \end{bmatrix} \begin{bmatrix}  \vert e, n-1\rangle \\ \vert g, n\rangle \end{bmatrix}\,.
\end{align}
...
\label{vstirap}
The preceding model can be extended to that of a $\Lambda$-like three-level system by introducing a ground state $\vert u, n-1 \rangle$ with energy $\hbar \omega_u$ that is coupled to the ex-
\begin{wrapfigure}[10]{l}{0.5\textwidth}
	...
	\end{tikzpicture}
	\caption[Energy level diagram of a $\Lambda$-like three-level system]{Energy level diagram of a $\Lambda$-like three-level system showing the laser detuning ($\Delta_L$), cavity detuning ($\Delta_C$) and Rabi frequencies ($\Omega$, $2g\sqrt{n}$) of the driving field and cavity, respectively.}
	\label{fig:3lvlLambdaSystem}
\end{wrapfigure}
cited state $\vert e, n-1 \rangle $ by a laser with a frequency of $\omega_L$ and a Rabi frequency of $\Omega$ (see \autoref{fig:3lvlLambdaSystem}). The atomic transition frequencies are denoted $\omega_{eu}=\omega_e-\omega_u$ and $\omega_{eg}=\omega_e-\omega_g$. The detuning between the driving field and the atomic resonance is therefore written as $\Delta_L=\omega_{eu}-\omega_L$. To distinguish the field parameters from those of the cavity, we occasionally write $\Delta=\Delta_C$ and $\omega=\omega_C$. Analogous to the two-level system, the Hamiltonian (in the rotating frame) of the three-level system can be decomposed into blocks that now couple the atom-field states $\{\vert u, n-1 \rangle,  \vert e,n-1\rangle, \vert g, n\rangle\}$ as
\begin{align}
\label{hamiltonianHn}
\begin{split}
\hat{\mathcal{H}}_n \begin{bmatrix}  \vert u, n-1 \rangle \\ \vert e, n-1\rangle \\ \vert g, n\rangle \end{bmatrix} = \hbar \begin{bmatrix} n\omega_C & \Omega/2 & 0 \\ \Omega/2 & n\omega_C + \Delta_L & g\sqrt{n} \\ 0 & g\sqrt{n} &  n\omega_C + \Delta_L-\Delta_C \end{bmatrix} \begin{bmatrix} \vert u, n-1 \rangle \\ \vert e, n-1\rangle \\ \vert g, n\rangle \end{bmatrix}\,.
\end{split}
\end{align}
...
\begin{figure}[!t]
	...
	\end{tikzpicture}
	\caption[Time evolution of a three-level atom-cavity system in the strong coupling regime]{Time evolution of the cavity emission rate \textsc{(left)} and atomic state populations \textsc{(right)} for a three level system (\autoref{fig:3lvlLambdaSystem}) in the strong coupling regime. The driving pulse has a temporal profile of $\Omega(t)=g\sin^2(\pi t/\SI{2}{\micro\second})$ and is shown as a dashed line on the left. The initial state of the system is $\vert u, 0 \rangle$ and the relevant parameters are given by $\Delta_L=\Delta_C=0$ and $(g,\gamma,\kappa)=2\pi\times(15,3,2)\,\si{\mega\hertz}$. The total photon emission probability from the cavity is \SI{95}{\percent} and the amount of energy dissipated in the form of spontaneously emitted photons lost to the environment amounts to \SI{5.4}{\percent} of a single excitation.}
...
\begin{figure}[t]
	...
	\end{tikzpicture}
	\caption[Time evolution of a three-level atom-cavity system in the bad cavity regime]{Time evolution of the cavity emission rate \textsc{(left)} and atomic state populations \textsc{(right)} for a three level system (\autoref{fig:3lvlLambdaSystem}) in the bad cavity regime. The driving pulse has a temporal profile of $\Omega(t)=g\sin^2(\pi t/\SI{2}{\micro\second})$ and is shown as a dashed line on the left. The initial state of the system is $\vert u, 0 \rangle$ and the relevant parameters are given by $\Delta_L=\Delta_C=0$ and $(g,\gamma,\kappa)=2\pi\times(15,3,20)\,\si{\mega\hertz}$. The total photon emission probability from the cavity is \SI{76}{\percent} and the amount of energy dissipated in the form of spontaneously emitted photons lost to the environment amounts to \SI{41}{\percent} of a single excitation.}
