...
\end{equation}
Through the scattering rate, this force depends on the detuning $\Delta$, which contains a velocity dependent component due to the Doppler shift experienced by moving atoms. This velocity-dependent scattering force is the underlying principle of Doppler cooling (see \autoref{dopplercooling}). In addition, the scattering force may acquire a positional dependence in the presence of a spatially varying magnetic field, which alters the detuning via the Zeeman effect. This position-dependent force, in combination with Doppler cooling, is what allows a magneto-optical trap (MOT) to cool and confine atoms (see \autoref{mots}).
\subsection{Doppler cooling}
\label{dopplercooling}
Doppler cooling is a technique used to reduce the kinetic energy of atoms, which enables confinement in conservative potentials like optical dipole traps (see \autoref{dipoleforce}). When an atom travels at a velocity $v$ facing a counter-propagating laser with wavenumber $k$, the angular frequency $\omega$ of the incident laser light is Doppler-shifted by $kv$: $\omega \rightarrow \omega + kv$. In a one-dimensional setup with two counter-propagating laser beams, the net scattering force $F_\text{net}$ acting on a moving atom can thus be expressed\footnote{This description is only valid for laser intensities well below saturation ($I/I_\text{sat}\ll 1$), in which case the scattering from each beam can be treated independently.} and expanded as:
\begin{align}
\begin{split}
	F_\text{net} &= F_\text{sc}(\omega_0-\omega+kv) - F_\text{sc}(\omega_0-\omega-kv) \\&= -\underbrace{\frac{16  \hbar k^2 \Gamma\Delta\Omega^2 }{(\Gamma^2 + 4 \Delta^2 +  2 \Omega^2)^2}}_{\alpha}v + \mathcal{O}(v^3) \\&\approx -\alpha v\,.
\end{split}
\label{coolingforce}
\end{align}
