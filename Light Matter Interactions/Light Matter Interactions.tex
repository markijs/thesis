...
\label{vstirap}
The preceding model can be extended to that of a $\Lambda$-like three-level system by introducing a ground state $\vert u, n-1 \rangle$ with energy $\hbar \omega_u$ that is coupled to the excited state $\vert e, n-1 \rangle $ by a laser with a frequency of $\omega_L$ and a Rabi frequency of $\Omega$ (see \autoref{fig:3lvlLambdaSystem}). The atomic transition frequencies are denoted $\omega_{eu}=\omega_e-\omega_u$
