...
\end{align}
such that $2\Omega^2/\Gamma^2=I/I_\text{sat}$. Note from \autoref{rhoee} that for $\Omega^2\gg\Gamma^2,\Delta^2$, we have $R_\text{sc}\approx\sfrac{\Gamma}{2}$, which is the regime for cooling lasers. Dipole trapping lasers typically operate at a large detuning ($\Delta^2\gg\Omega^2,\Gamma^2$), in which case $R_\text{sc}\approx\sfrac{\Gamma\Omega^2}{4\Delta^2}$. To conclude, the scattering force is given by the product of the photon momentum $\hbar k$ and the scattering rate:
\begin{equation}
...
\label{coolingforce}
\end{align}
From \autoref{coolingforce}, it follows that red-detuned laser light ($\Delta>0$), for which $\alpha$ is positive, exerts a frictional or damping force on the atom, effectively reducing its velocity and temperature. For $\Delta=\sfrac{\Gamma}{2}$ and $\Omega=\Gamma$, the damping coefficient $\alpha$ attains its maximum of $\alpha_\text{max}=\sfrac{\hbar k^2}{2}$.

\begin{figure}[!t]
	\hspace{-0.2em}
	\begin{tikzpicture} 
	\begin{groupplot}[group style={group size=2 by 1,horizontal sep=3.7cm, vertical sep=0.5cm},width=6cm, height=6cm]
	\nextgroupplot
	[
	...
	\addplot graphics [xmin=-2,xmax=2,ymin=0,ymax=3] {doppler.pdf};
	...
	\pgfmathdeclarefunction{F}{2}{% #1=Delta, #2=velocity
		\pgfmathparse{
			(1/(3+ 4*(#1 + #2)^2) - 
			(1/(3 + 4*(#1 - #2)^2)
		}
	...
	\pgfmathdeclarefunction{Fleft}{2}{% #1=Delta, #2=velocity
	\pgfmathparse{
		(1/(3+ 4*(#1 + #2)^2)
		}
	}

	\pgfmathdeclarefunction{Fright}{2}{% #1=Delta, #2=velocity
	\pgfmathparse{ -1/(3 + 4*(#1 - #2)^2)
		}
	}
	
	% Plot for Delta = Gamma/2 (optimal cooling)
	\addplot[rainbow1of8, thick, domain=-3:3] {F(1/2, x)};
	\addplot[rainbow1of8, dashed, domain=-3:3] {Fleft(1/2, x)};
	\addplot[rainbow1of8, dashed, domain=-3:3] {Fright(1/2, x)};

	
	% Plot for Delta = Gamma (suboptimal)
	\addplot[rainbow2of8, thick, domain=-3:3] {F(1, x)};
	\addplot[rainbow2of8, dashed, domain=-3:3] {Fleft(1, x)};
	\addplot[rainbow2of8, dashed, domain=-3:3] {Fright(1, x)};
	
	% Plot for Delta = Gamma (suboptimal)
	\addplot[rainbow3of8, thick, domain=-3:3] {F(2, x)};
	\addplot[rainbow3of8, dashed, domain=-3:3] {Fleft(2, x)};
	\addplot[rainbow3of8, dashed, domain=-3:3] {Fright(2, x)};
	
	\legend{\footnotesize\hspace{0.3em}$\Delta = \sfrac{\Gamma}{2}$, , ,\footnotesize\hspace{0.3em}$\Delta = \Gamma$, , ,\footnotesize\hspace{0.3em}$\Delta = 2\Gamma$}
	...
	\end{groupplot}
	\end{tikzpicture}
	\caption[Damping coefficient and net cooling force in a one-dimensional optical molasses]{(Left) Dimensionless damping coefficient $\sfrac{\alpha}{\hbar k^2}$ in a one-dimensional optical molasses as a function of $\sfrac{\Omega}{\Gamma}$ and $\sfrac{\Delta}{\Gamma}$. The maximum is located at $\Omega = \Gamma$ and $\Delta = \sfrac{\Gamma}{2}$. (Right) Net cooling force $\sfrac{F_\text{net}}{\hbar k \Gamma}$ versus velocity $\sfrac{v}{(\Gamma k^{-1} )}$ (both dimensionless) for $\Omega = \Gamma$ and $\Delta\in\{\sfrac{\Gamma}{2}, \Gamma, 2\Gamma\}$. The antisymmetric profile is characteristic of Doppler cooling, exhibiting a linear dependence near $v = 0$ and saturation at higher velocities. Dashed curves represent the forces exerted by a single cooling beam.
	}
	\label{dopplerplots} 
\end{figure}
