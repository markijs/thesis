...
\begin{figure}[!t]
	...
	\end{groupplot}
	\end{tikzpicture}
	\caption[Damping coefficient and net cooling force in a one-dimensional optical molasses]{(Left) Dimensionless damping coefficient $\sfrac{\alpha}{\hbar k^2}$ in a one-dimensional optical molasses as a function of $\sfrac{\Omega}{\,\Gamma}$ and $\sfrac{\Delta}{\,\Gamma}$. For fixed values of $\sfrac{\Omega}{\,\Gamma}$, the minima and maxima are shown as white and black dashed lines, respectively. (Right) Net cooling force $\sfrac{F_\text{net}}{\hbar k \Gamma}$ versus velocity $\sfrac{v}{(\Gamma k^{-1} )}$ (both dimensionless) for ${\Omega}=\sfrac{\Gamma}{2}$ and $\Delta\in\{\sfrac{\Gamma}{10},\sfrac{\Gamma}{2}, \Gamma, 2\Gamma\}$. The antisymmetric profile is characteristic of Doppler cooling, exhibiting a linear dependence near $v = 0$ and saturation at higher velocities. Dashed curves represent the forces exerted by a single cooling beam.
	}
	\label{dopplerplots} 
\end{figure}
