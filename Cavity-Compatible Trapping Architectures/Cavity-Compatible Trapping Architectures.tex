...
\begin{figure}[!t]
	...
	\node[markgrijs, align=center, anchor=center] at (0.63, 0.5) {\footnotesize $=$};
	
	\end{scope}
	
	\end{tikzpicture}
	\caption[asdf]{Schematic of the pyramidal MOT mirror configuration with an aperture at the apex. The front view shows the four reflective faces around the square central aperture. The side view illustrates the incident and reflected \SI{780}{\nano\meter} laser beam interacting with the aperture. The diffraction of the reflected beam due to the finite aperture size is equivalently described as diffraction from a square obstruction of size $a$ (right). The diffraction pattern is modelled at various distances $d$.}
