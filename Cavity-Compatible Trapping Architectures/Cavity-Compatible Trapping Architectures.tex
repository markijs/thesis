...
\begin{figure}
	...
	\begin{tikzpicture}
			...
			%\draw[] (axis cs:163,0.190) rectangle (axis cs:182,0.360);
		...
		\end{axis}
	\end{tikzpicture}
	\caption[Loading an intracavity dipole trap from a pyramid MOT]{(Left) Schematic of the right-angle mirror substrate with dimensions in millimetres. The end facet length 
		a
		a and adjacent length 
		b
		b are indicated, with the gold-coated surface highlighted in yellow. The red volume represents the material to be removed, which depends on 
		a
		a. (Right) Measured end facet length 
		a
		a as a function of polishing time. Data points show measured values with error bars. The solid line is a fit based on Preston’s law, which assumes a constant material removal volume per unit time. The only fit parameter is the material removal rate, found to be 
		0.0128
