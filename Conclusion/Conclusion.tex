...
The work presented in this thesis was undertaken with the objective of combining studies of optical dipole trapping, holographic beam shaping and Fabry--Pérot resonator engineering. The result is a set of mutually compatible tools which enable scalable atom–photon interfacing for quantum networks.
...
By increasing the depth of an optical dipole trap, the transition from fast loading to long-lived trapping was demonstrated. Loading was achieved on a timescale of \SI{2.1}{\second} followed by a measured lifetime of \SI{7.9}{\second}, yielding a single-site filling fraction of \SI{79\pm 2}{\percent} without atom rearrangement.  This result surpasses the conventional collisional-blockade limit of \SI{50}{\percent}, offering a practical route to defect-free arrays.

\item \emph{Power efficient holograms for homogeneous trap arrays.}  
A systematic comparison of iterative Fourier transform algorithms demonstrated that weighted Gerchberg--Saxton (GSW) and hybrid methods that combine GSW with the mixed region amplitude freedom (MRAF) algorithm achieved the best overall performance for $4\times4$ trap arrays. The GSW approach maintained a power efficiency of about \SI{90}{\percent}, a trap intensity coefficient of variation as low as \num{4.2e-7} and the fastest convergence time of \SI{0.8}{\second} among all tested algorithms. These metrics support a reliable computational framework for generating uniform, reconfigurable trap arrays.

\item \emph{Realisation of a high-finesse Fabry--Pérot resonator for strong coupling.}  
The assembled resonator, which consists a high-transmission near-planar output mirror ($T=\SI{38.2}{ppm}$) and a low-transmission curved micro-mirror ($T=\SI{1.74}{ppm}$), exhibited a finesse of $\num{7.51(9)e4}$ and a linewidth of $\SI{4.20}{\mega\hertz}$. The corresponding coupling parameters $(g_{\mathrm{max}},\gamma,\kappa)=2\pi\times(11.5,3.03,2.10)~\mathrm{MHz}$ yield a single-atom cooperativity $C = 10.4$, firmly placing our resonator in the strong-coupling regime for atom-photon interfacing.
...
maybe even reflect back on fundametal tests of reality
